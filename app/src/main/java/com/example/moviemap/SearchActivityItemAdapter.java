...
public class SearchActivityItemAdapter extends RecyclerView.Adapter<SearchActivityItemAdapter.ItemViewHolder> implements Filterable {

    private List<SearchActivityItemModel> mDataList;
    private List<SearchActivityItemModel> mDataListAll;

    //constructor
    public SearchActivityItemAdapter(List<SearchActivityItemModel> items, SearchActivity searchActivity) {
        mDataList = items;
    ...
    public void setOnItemListener(SearchActivityItemAdapter.onItemListener listener) {
        mListener = listener;
    }

    //data set changed
    public void dataSetChanged(List<SearchActivityItemModel> exampleList) {
    ...
    public void onBindViewHolder(@NonNull final ItemViewHolder holder, final int position) {
        SearchActivityItemModel currentItem = mDataList.get(position);
    ...
    private Filter exampleFilter = new Filter() {
                ...
                protected FilterResults performFiltering(CharSequence constraint) {
                    List<SearchActivityItemModel> filteredList = new ArrayList<>();

                    if (constraint == null || constraint.length() == 0) {
                        filteredList.addAll(mDataListAll);
                    } else {
                        String filterPattern = constraint.toString().toLowerCase().trim();
                for (SearchActivityItemModel item : mDataListAll) {
