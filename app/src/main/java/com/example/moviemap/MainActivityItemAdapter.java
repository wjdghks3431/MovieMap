...
public class MainActivityItemAdapter extends RecyclerView.Adapter<MainActivityItemAdapter.ItemViewHolder> implements Filterable {

    private List<MainActivityItemModel> mDataList;
    private List<MainActivityItemModel> mDataListAll;

    //constructor
    public MainActivityItemAdapter(List<MainActivityItemModel> items, MainActivity mainActivity) {
        mDataList = items;
    ...
    //data set changed
    public void dataSetChanged(List<MainActivityItemModel> exampleList) {
    ...
    public void onBindViewHolder(@NonNull final ItemViewHolder holder, final int position) {
        MainActivityItemModel currentItem = mDataList.get(position);
    ...
    private Filter exampleFilter = new Filter() {
                ...
                protected FilterResults performFiltering(CharSequence constraint) {
                    List<MainActivityItemModel> filteredList = new ArrayList<>();

                    if (constraint == null || constraint.length() == 0) {
                        filteredList.addAll(mDataListAll);
                    } else {
                        String filterPattern = constraint.toString().toLowerCase().trim();
                for (MainActivityItemModel item : mDataListAll) {
